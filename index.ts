...
class DrawingUtil {
    ...
    static drawTriExpandDrop(context : CanvasRenderingContext2D, scale : number) {
        ...
        for (var j = 0; j < lines; j++) {
            context.save()
            context.translate(0, -size / 2 + size * 0.5 * j)
            DrawingUtil.drawLine(context, -size * 0.5 * sc2, 0, size * sc2 * 0.5, 0)
            context.restore()
        }
        context.restore()
    }
...
class TEDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new TEDNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawTEDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : TEDNode {
        var curr : TEDNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
